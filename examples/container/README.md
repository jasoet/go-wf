...
This directory contains comprehensive examples demonstrating the capabilities of the `go-wf/container` package for orchestrating container workflows with Temporal.
...
1. Start the worker in a separate terminal:
   ...
   - Listen on the `container-tasks` task queue
...
All examples use the **`container-tasks`** task queue. Make sure your worker is listening on this queue:
...
**Error**: `no Go files in /path/to/examples/container`
...
client.StartWorkflowOptions{
    TaskQueue: "container-tasks",  // Must match worker
}
```
...
- Review the [main container package README](../../container/README.md) for API documentation
- Explore the [source code](../../container/) to understand implementation details
