...
Temporal workflows for executing containers with advanced orchestration patterns. This package provides Argo Workflow-like capabilities using Temporal, with builder patterns, pre-built workflow templates, and lifecycle management.

## Features

### Core Workflows
- **Single Container Execution** - Run individual containers with full configuration
...
```bash
go get github.com/jasoet/go-wf/container
```
...
func main() {
    ...
    we, _ := c.ExecuteWorkflow(context.Background(),
        ...
        container.ExecuteContainerWorkflow,
        input)

    var result container.ContainerExecutionOutput
...
```go
import "github.com/jasoet/go-wf/container/patterns"
...
```go
import "github.com/jasoet/go-wf/container"

// Define nodes with dependencies
input := container.DAGWorkflowInput{
    Nodes: []container.DAGNode{
        {
            Name: "checkout",
            Container: container.ExtendedContainerInput{
                ContainerExecutionInput: container.ContainerExecutionInput{
                    Image:   "alpine/git",
...
we, _ := c.ExecuteWorkflow(ctx,
    ...
    container.DAGWorkflow,
    input)

var output container.DAGWorkflowOutput
we.Get(ctx, &output)
```
...
// Check status later
status, err = container.GetWorkflowStatus(ctx, temporalClient, workflowID, runID)
```
...
go container.WatchWorkflow(ctx, temporalClient, workflowID, runID, updates)
...
err := container.CancelWorkflow(ctx, temporalClient, workflowID, runID)

// Terminate workflow (immediate stop)
err := container.TerminateWorkflow(ctx, temporalClient, workflowID, runID, "reason")
```
...
err := container.SignalWorkflow(ctx, temporalClient, workflowID, runID, "pause", nil)

// Query workflow state
var state string
err := container.QueryWorkflow(ctx, temporalClient, workflowID, runID, "status", &state)
```
...
params := []container.WorkflowParameter{
    {Name: "version", Value: "v1.2.3"},
    {Name: "env", Value: "production"},
}

output, err := container.WorkflowWithParameters(ctx, input, params)
```
...
node := container.DAGNode{
    ...
    Container: container.ExtendedContainerInput{
        ...
        Conditional: &container.ConditionalBehavior{
...
container := container.ExtendedContainerInput{
    ...
    InputArtifacts: []container.Artifact{
        {Name: "source", Path: "/src", Type: "directory"},
    },
    OutputArtifacts: []container.Artifact{
...
container := container.ExtendedContainerInput{
    ...
    Secrets: []container.SecretReference{
...
```go
input := container.PipelineInput{
    StopOnError: true,
    Containers: []container.ContainerExecutionInput{
...
go test -tags=integration ./container/...

# Coverage
go test -coverprofile=coverage.out ./container/...
```
...
| Argo Workflows | go-wf/container | Example |
|---------------|--------------|---------|
...
See [examples/container/](../examples/container/) directory for complete working examples with comprehensive documentation.
...
For detailed documentation, see [examples/container/README.md](../examples/container/README.md).
