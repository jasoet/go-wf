...
func TestSubmitWorkflow(t *testing.T) {
	...
	status, err := SubmitWorkflow(context.Background(), mockClient, input, "container-queue")
...
func TestSubmitAndWait(t *testing.T) {
	...
	status, err := SubmitAndWait(context.Background(), mockClient, input, "container-queue", 1*time.Minute)
	assert.NoError(t, err)
	assert.NotNil(t, status)
...
func TestSubmitWorkflowWithDifferentInputTypes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			status, err := SubmitWorkflow(context.Background(), mockClient, tt.input, "container-queue")
			assert.NoError(t, err)
			assert.NotNil(t, status)
		})
	}
...
func TestSubmitWorkflow_UnsupportedType(t *testing.T) {
	...
	status, err := SubmitWorkflow(context.Background(), mockClient, "invalid-input", "container-queue")
	assert.Error(t, err)
	assert.Nil(t, status)
